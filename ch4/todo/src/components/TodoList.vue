<template>
  <main>
    <div v-for="(todo, idx) in data" :key="todo.id">
      ...
      <div v-show="idx + 1 < data.length" class="col border border-second" /><!-- 이런것은 처음이네 <hr /> 만 쓰다가. -->
    </div>
  </main>
</template>
...
export default {
  ...
  props: {
    data: {
      type: Array, // Vue 에서는 왜 type 을 표준이 아닌 것으로 썼지? 머. typeof('hello') 는 'string' 인데, 문서에서는 [String] 으로도 표기되어 있네.
      default: [],
    }
  },
  setup() {
